<template>
  <div class="bg-[#F2F2F2] w-full">
    <div
      class="w-[1440px] xl-max:w-full md-max:pt-[6px] lg-max:w-full mx-auto gap-6 px-6 md-max:px-[16px] flex lg:flex-row flex-col items-center py-[56px]"
    >
      <div
        class="py-4 lg:py-10 flex-col justify-start items-start gap-4 inline-flex lg:w-1/2 flex-1"
      >
        <div class="flex-col justify-start items-start gap-2.5 flex">
          <div
            class="font-[500] text-[18px] heading leading-relaxed space-y-2 sm:text-[24px] md:text-[24px]"
          >
            Have more questions?
          </div>
          <div
            class="text-[13px] sm:text-sm md:text-[15px] lg:text-[17px] leading-[23px] sm:leading-6 md:leading-[27px] lg:leading-[30px] font-[300] pt-3"
          >
            If you have a question that wasn’t answered in our FAQ, don’t
            hesitate to reach out to us. We’re here to help! Simply fill out the
            form below, and our support team will get back to you as soon as
            possible.
          </div>
        </div>
        <div class="flex flex-row justify-start items-start gap-2.5">
          <div
            class="text-[#005b52] text-[14px] font-bold font-['Noto Sans JP']"
          >
            Ask us your question
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
          >
            <g clip-path="url(#clip0_1163_44719)">
              <path
                d="M23.12 9.9099L19.25 5.9999C19.157 5.90617 19.0464 5.83178 18.9246 5.78101C18.8027 5.73024 18.672 5.7041 18.54 5.7041C18.408 5.7041 18.2773 5.73024 18.1554 5.78101C18.0336 5.83178 17.923 5.90617 17.83 5.9999C17.6437 6.18726 17.5392 6.44071 17.5392 6.7049C17.5392 6.96909 17.6437 7.22254 17.83 7.4099L21.39 10.9999H1C0.734784 10.9999 0.48043 11.1053 0.292893 11.2928C0.105357 11.4803 0 11.7347 0 11.9999H0C0 12.2651 0.105357 12.5195 0.292893 12.707C0.48043 12.8945 0.734784 12.9999 1 12.9999H21.45L17.83 16.6099C17.7363 16.7029 17.6619 16.8135 17.6111 16.9353C17.5603 17.0572 17.5342 17.1879 17.5342 17.3199C17.5342 17.4519 17.5603 17.5826 17.6111 17.7045C17.6619 17.8263 17.7363 17.9369 17.83 18.0299C17.923 18.1236 18.0336 18.198 18.1554 18.2488C18.2773 18.2996 18.408 18.3257 18.54 18.3257C18.672 18.3257 18.8027 18.2996 18.9246 18.2488C19.0464 18.198 19.157 18.1236 19.25 18.0299L23.12 14.1499C23.6818 13.5874 23.9974 12.8249 23.9974 12.0299C23.9974 11.2349 23.6818 10.4724 23.12 9.9099Z"
                fill="#005B52"
              />
            </g>
            <defs>
              <clipPath id="clip0_1163_44719">
                <rect width="24" height="24" fill="white" />
              </clipPath>
            </defs>
          </svg>
        </div>
      </div>

      <div
        class="md:p-[40px] p-[16px] bg-white lg:w-1/2 rounded-[10px] w-full lg:min-w-[580px] lg:max-w-[724px]"
      >
        <form @submit.prevent="handleSubmit" class="">
          <!-- Name Field -->
          <div class="flex flex-col gap-[24px]">
            <div class="flex sm:flex-row flex-col items-center w-full gap-6">
              <CusBaseInput
                v-model="form.name"
                label="Your Name"
                type="text"
                placeholder="Oudom"
                :errorMessage="errors.name"
              />
              <CusBaseInput
                v-model="form.email"
                label="Your Email"
                type="email"
                placeholder="oudom@gmail.com"
                :errorMessage="errors.name"
              />
            </div>

            <div class="flex flex-col gap-[16px]">
              <CusBaseInput
                v-model="form.message"
                label="Your Message"
                type="textarea"
                placeholder="How does your platform ensure that my payment and personal information are kept secure?"
                :errorMessage="errors.name"
              />
            </div>
          </div>

          <div
            class="flex flex-col-reverse md:flex-row md:gap-0 gap-[44px] md:items-center items-start justify-between w-full pt-[24px]"
          >
            <div class="w-auto h-auto">
              <img
                src="/img/Capture d’écran 2024-10-29 à 19.59.46 1.png"
                alt=""
                class="w-[301px] h-[74px]"
              />
            </div>
            <button
              style="box-shadow: 0px 4px 16px 0px #dbf226"
              type="submit"
              class="text-sm px-20 py-4 bg-[#212121] rounded-md shadow justify-end items-center text-white md:w-fit w-full whitespace-nowrap"
            >
              send
            </button>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>
<script setup lang="ts">
import CusBaseInput from '../cusComponents/CusBaseInput.vue'

// defineProps<{
//   topTitle: string
//   des?: string
//   link: string
//   listPa?: { para: string }[]
// }>()

interface JobDetail {
  key: string
  value: string
}

interface FormData {
  name: string
  email: string
  subject: string
  message: string
  cv: string
  letter: string
  portfolio: string
}

interface FormErrors {
  name?: string
  email?: string
  subject?: string
  message?: string
}

const discoverJob = ref<JobDetail[]>([
  {
    key: 'Location',
    value: 'Head office in Phnom Penh',
  },
  {
    key: 'Type of contract',
    value: 'Permanent',
  },
  {
    key: 'Salary',
    value: '$12,480 gross per year with 13th month bonus',
  },
  {
    key: 'Reports to',
    value: 'Chief Operating Officer (COO)',
  },
])

const form = reactive<FormData>({
  name: '',
  email: '',
  subject: '',
  message: '',
  cv: '',
  letter: '',
  portfolio: '',
})

const errors = ref<FormErrors>({})

const validateForm = (): boolean => {
  errors.value = {}

  if (!form.name.trim())
    errors.value.name = 'Special characters are not accepted'
  if (!form.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
    errors.value.email = 'Please enter a valid email address'
  if (!form.subject.trim()) errors.value.subject = 'Subject is required.'
  if (!form.message.trim()) errors.value.message = 'Message is required.'

  return Object.keys(errors.value).length === 0
}

const handleSubmit = () => {
  if (validateForm()) {
    alert('Form submitted successfully!')
    console.log(form)

    // Reset form fields
    Object.assign(form, {
      name: '',
      email: '',
      subject: '',
      message: '',
    })
  }
}
</script>
