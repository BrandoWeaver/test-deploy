<template>
  ...
  >
    ...
    >
      <div
        v-if="seasonalData?.length === 0 && pageSeasonal === 1"
        v-for="i in 5"
        :key="i"
        class="card w-[273px] h-[65px] flex-shrink-0 relative overflow-hidden rounded-lg shadow-md bg-gray-200 animate-pulse"
      >
        <div class="absolute inset-0 flex items-center justify-center"></div>
      </div>
      <div
        v-else
        v-for="(item, index) in seasonalData"
        :key="item.id || index"
      ...
      >
        ...
        ></div>
        <div
          class="absolute inset-0 flex items-center justify-center text-white font-semibold text-sm z-[1] text-center"
        >
          {{ item.seasonal_date }}
        </div>
      </div>
      <div v-if="isFetchingSeasonal"><CusLoading width="20" height="20" /></div>
    </div>
  </div>
</template>
...
const carousel = ref<HTMLElement | null>(null) // Explicitly type as HTMLElement or null

const props = defineProps<{
  seasonalData?: ISEASONAL.Daum[]
  status: boolean
  loadMoreSeasonal?: () => void
  isFetchingSeasonal?: boolean
  pageSeasonal?: number
}>()

function scrollLeft() {
  if (carousel.value) {
    // Ensure the carousel has a first child to get its offsetWidth
    const firstCard = carousel.value.querySelector('.card') as HTMLElement
    if (firstCard) {
      const scrollAmount = firstCard.offsetWidth + 14
...
// --- Pagination Logic ---
const handleScroll = () => {
  if (!carousel.value || props.isFetchingSeasonal || !props.loadMoreSeasonal) {
    return
  }

  const { scrollWidth, scrollLeft, clientWidth } = carousel.value
  const scrollThreshold = 200 // Load more data when within 200px of the end

  // Check if the user has scrolled near the end
  if (scrollLeft + clientWidth >= scrollWidth - scrollThreshold) {
    props.loadMoreSeasonal()
  }
}
