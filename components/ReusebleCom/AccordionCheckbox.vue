<template>
  ...
  >
    <div v-show="isOpen" class="w-full flex flex-col gap-3 overflow-hidden">
      <div v-for="(item, index) in asset" :key="index" class="flex">
        <label class="inline-flex items-center" for="redCheckBox">
          <input
            ...
            id="checkbox-{{ item.id }}"
            :style="{
              borderColor: selectedItems.includes(item) ? 'green' : 'red',
            }"
            class="w-4 h-4 accent-green-dark border-2 rounded-md cursor-pointer"
          />
          <span class="ml-3">{{ item.lable }}</span>
        </label>
      </div>
    </div>
  </transition>
...
const selectedItems = ref<{ name: string; id: number }[]>([])
