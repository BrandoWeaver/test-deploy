<template>
  <div class="w-full bg-white pb-8">
    ...
    >
      <div
        v-for="(item, index) in listRelatedImage?.data"
        :key="index"
      ...
      >
        <div class="relative">
          <!-- Top Section -->
          <div
            class="absolute top-0 left-0 flex-shrink flex-col gap-[4px] opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          >
            <!-- AI Label and Title -->
...
const formatTags = computed(() => props.tags.map((tag) => tag.id).join(','))
console.log('props.tags', formatTags.value)
...
const { data: listRelatedImage } = useApi<IRELATEIMAGE.RelateImage>(
  () =>
      ...
      .replace(':lang', locale.value),
  {
    ...
    immediate: true,
    onSuccess(data) {
      console.log('listRelatedImage', data)
    },
  }
)
</script>
