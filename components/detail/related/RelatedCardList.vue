<template>
  <div class="w-full bg-white pb-8">
    ...
    <div
      ref="carousel"
      class="relative justify-center w-full items-center flex overflow-x-auto no-scrollbar gap-4 px-8 xl-max:px-6 xl-max:gap-[12px] lg-max:gap-[10px]"
    >
      <div
        v-if="
          $nuxt.ssrContext ||
          (status === 'pending' && props.page_number === 1) ||
          listRelatedImage?.length === 0
        "
      ...
      >
        <div
          ...
          v-for="n in 20"
          :key="'skeleton-' + n"
          class="product-card group w-[300px] bg-[#F5F5F5] rounded-lg p-2"
        >
          <div class="w-full h-[150px] bg-[#F5F5F5] rounded"></div>
        </div>
      </div>
      <div
        v-else
        v-for="(item, index) in listRelatedImage"
        :key="index"
        class="bg-blue-50 group transition-all duration-300 ease-in-out flex-shrink-0 flex items-center justify-center"
        @mouseenter="playVideo(index)"
        @mouseleave="pauseVideo(index, item.section.id)"
      >
        <NuxtLink
          :to="`/all-products/${item.section.slug}/${item.sub_section.slug}/${item.categories[0].slug}/${item.collections[0].slug}/${item.slug}`"
          class="relative"
          @click="trackClick(item.section.id, props.content_id)"
        >
          <div
            class="absolute z-50 top-0 left-0 flex-shrink flex-col gap-[4px] opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          >
            <div class="flex items-center gap-[2px]">
              ...
              <span
                class="px-2 py-1 text-[8px] font-bold text-white bg-[rgba(0,0,0,0.5)] rounded-[2px]"
              >
                {{ item.title }}
              </span>
            </div>
            ...
            <span
              class="px-2 py-1 text-[8px] font-medium text-white bg-[rgba(0,0,0,0.5)] rounded-[2px]"
            >
              By {{ item.creator.name.toLocaleUpperCase() }}
            </span>
          </div>
          <div class="flex flex-row items-center space-x-6 h-full">
            <CusComponentsImageContainer
              :key="item.id"
              :image-url="item.thumbnail || ''"
              :alt-text="item.thumbnail || ''"
            ...
            <video
              ...
              playsinline
              :poster="''"
              @loadedmetadata="(e) => logDuration(e, index)"
            ></video>
          </div>
          <div
            class="absolute bottom-0 flex justify-between w-full items-end opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          >
            <div
            ...
            <div class="flex flex-col justify-end items-end gap-2">
              <div
                class="flex items-center h-[19px] justify-between gap-[3px] px-[4px] py-[2px] bg-[rgba(0,0,0,0.5)] rounded text-white"
                v-if="item.on_sale"
              >
                <div class="line-through text-[8px] text-white/70">$35</div>
                <div class="font-bold text-[12px]">$35</div>
              </div>
              <div
                class="flex h-[19px] gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
              >
                <button
...
const props = defineProps<{
  listRelatedImage?: IRELATEIMAGE.dataRelateImage[]
  status?: string
  loadMore?: () => void
  isFetching?: boolean
  page_number?: number
...
const pauseVideo = (index: number, section_id?: number) => {
  const video = videoRefs.value[index]
  if (video) {
    video.pause()
    video.currentTime = 0
  }
  endHover('product-card-in-detail-page', section_id!, props.content_id)
}
