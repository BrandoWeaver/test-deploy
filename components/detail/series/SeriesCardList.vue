<template>
  ...
  <div class="w-full bg-white pb-8">
    ...
    <div
      ref="carousel"
      class="relative justify-center w-full flex items-center overflow-x-auto no-scrollbar gap-4 px-8 xl-max:px-6 xl-max:gap-[12px] lg-max:gap-[10px]"
    >
      <div
        v-if="
          $nuxt.ssrContext ||
          (status && props.page_number === 1) ||
          listCardSeries?.length === 0
        "
      ...
      <div
        v-else
        v-for="(item, index) in listCardSeries"
        :key="index"
      ...
      >
        <NuxtLink
          ...
          :to="`/all-products/${item?.section?.slug || 'unknown'}/${item?.sub_section?.slug || 'unknown'}/${item?.categories[0]?.slug || 'unknown'}/${item?.collections[0]?.slug || 'unknown'}/${item?.slug || 'unknown'}`"
        >
          <div
...
const props = defineProps<{
  section_id?: number
  content_id?: number
  listCardSeries?: IImageSeries.ImageSeries[]
  status?: boolean
  loadMore?: () => void
  isFetching?: boolean
  page_number?: number
